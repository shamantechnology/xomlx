...
from xomlx.helpers import xomlx_text, pretty_print_bytes, pretty_print_bytes_per_second
from xomlx.topology.topology import Topology
from xomlx.topology.partitioning_strategy import Partition
from xomlx.download.download_progress import RepoProgressEvent
from xomlx.topology.device_capabilities import UNKNOWN_DEVICE_CAPABILITIES
...
class TopologyViz:
  ...
  def _generate_main_layout(self) -> str:
    ...
    # Add xomlx_text at the top in red
    xomlx_lines = xomlx_text.split("\n")
    red_style = Style(color="red1")
    max_line_length = max(len(line) for line in xomlx_lines)
    for i, line in enumerate(xomlx_lines):
    ...
    info_start_y = len(xomlx_lines) + 1
