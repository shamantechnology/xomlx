...
from xomlx.train.dataset import load_dataset, iterate_batches
from xomlx.networking.manual.manual_discovery import ManualDiscovery
from xomlx.orchestration.node import Node
from xomlx.networking.grpc.grpc_server import GRPCServer
from xomlx.networking.udp.udp_discovery import UDPDiscovery
from xomlx.networking.grpc.grpc_peer_handle import GRPCPeerHandle
from xomlx.topology.ring_memory_weighted_partitioning_strategy import RingMemoryWeightedPartitioningStrategy
from xomlx.api import ChatGPTAPI
from xomlx.download.shard_download import ShardDownloader, NoopShardDownloader
from xomlx.download.download_progress import RepoProgressEvent
from xomlx.download.new_shard_download import new_shard_downloader, has_xomlx_home_read_access, has_xomlx_home_write_access, ensure_xomlx_home, seed_models
from xomlx.helpers import print_red_xomlx, find_available_port, DEBUG, get_system_info, get_or_create_node_id, get_all_ip_addresses_and_interfaces, terminal_link, shutdown
from xomlx.inference.shard import Shard
from xomlx.inference.inference_engine import get_inference_engine
from xomlx.inference.tokenizers import resolve_tokenizer
from xomlx.models import build_base_shard, get_repo
from xomlx.viz.topology_viz import TopologyViz
from xomlx.viz.chat_tui import run_chat_tui
...
parser.add_argument("--data", type=str, default="xomlx/train/data/lora", help="Directory where training data lives")
...
parser.add_argument("--inference-engine", type=str, default="mlx", help="Inference engine to use (mlx or dummy)")
...
async def check_xomlx_home():
  home, has_read, has_write = await ensure_xomlx_home(), await has_xomlx_home_read_access(), await has_xomlx_home_write_access()
  if DEBUG >= 1: print(f"xomlx home directory: {home}")
  print(f"{has_read=}, {has_write=}")
  if not has_read or not has_write:
    print(f"""
          WARNING: Limited permissions for xomlx home directory: {home}.
...
async def main():
  ...
  try: await check_xomlx_home()
  except Exception as e: print(f"Error checking xomlx home directory: {e}")
