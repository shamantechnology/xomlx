...
import traceback
import signal
from xomlx import DEBUG, VERSION
from xomlx.helpers import PrefixDict, shutdown, get_uuid_images_dir
from xomlx.inference.tokenizers import resolve_tokenizer
from xomlx.orchestration import Node
from xomlx.models import build_base_shard, build_full_shard, model_cards, get_repo, get_supported_models, get_pretty_name
...
from xomlx.download.new_shard_download import delete_model
...
def generate_completion(
  ...
  completion = {
    ...
    "system_fingerprint": f"xomlx_{VERSION}",
    "choices": [{
      "index": 0,
...
class ChatGPTAPI:
  ...
  async def handle_get_models(self, request):
    models_list = [{"id": model_name, "object": "model", "owned_by": os.getenv("XOMLX_UUID", "self"), "ready": True} for model_name, _ in model_cards.items()]
