...
from xomlx.models import get_repo
from pathlib import Path
from xomlx.download.hf.hf_helpers import get_hf_endpoint, get_auth_headers, filter_repo_objects, get_allow_patterns
from xomlx.download.shard_download import ShardDownloader
from xomlx.download.download_progress import RepoProgressEvent, RepoFileProgressEvent
from xomlx.helpers import AsyncCallbackSystem, DEBUG
from xomlx.models import get_supported_models, build_full_shard
import os
...
def xomlx_home() -> Path:
  return Path(os.environ.get("XOMLX_HOME", Path.home()/".cache"/"xomlx"))

def xomlx_tmp() -> Path:
  return Path(tempfile.gettempdir())/"xomlx"

async def ensure_xomlx_home() -> Path:
  await aios.makedirs(xomlx_home(), exist_ok=True)
  return xomlx_home()

async def ensure_xomlx_tmp() -> Path:
  await aios.makedirs(xomlx_tmp(), exist_ok=True)
  return xomlx_tmp()

async def has_xomlx_home_read_access() -> bool:
  try: return await aios.access(xomlx_home(), os.R_OK)
  except OSError: return False

async def has_xomlx_home_write_access() -> bool:
  try: return await aios.access(xomlx_home(), os.W_OK)
  except OSError: return False

async def ensure_downloads_dir() -> Path:
  downloads_dir = xomlx_home()/"downloads"
...
async def fetch_file_list_with_cache(repo_id: str, revision: str = "main") -> List[Dict[str, Union[str, int]]]:
  cache_file = (await ensure_xomlx_tmp())/f"{repo_id.replace('/', '--')}--{revision}--file_list.json"
...
async def get_weight_map(repo_id: str, revision: str = "main") -> Dict[str, str]:
  target_dir = (await ensure_xomlx_tmp())/repo_id.replace("/", "--")
