...
Path().cwd().absolute().joinpath("logs").mkdir(exist_ok=True)
LOG_PATH = Path().cwd().absolute().joinpath("logs", f"run_{datetime.now().strftime('%Y_%m_%d')}.log")
...
def find_available_port(host: str = "", min_port: int = 49152, max_port: int = 65535) -> int:
  used_ports_file = os.path.join(tempfile.gettempdir(), "xomlx_used_ports")
...
def get_or_create_node_id():
  NODE_ID_FILE = Path(tempfile.gettempdir())/".xomlx_node_id"
  try:
...
def is_frozen():
  return getattr(sys, 'frozen', False) or os.path.basename(sys.executable) == "xomlx" \
...
def get_uuid_home() -> Path:
  if os.getenv("XOMLX_UUID") is not None:
    xomlx_uuid = os.getenv("XOMLX_UUID")
  else:
    xomlx_uuid = str(uuid.uuid4()).replace('-', '')
    os.environ["XOMLX_UUID"] = xomlx_uuid
  ...
  uuid_folder = docs_folder/xomlx_uuid
